...
function initialize() {
  ...
  try {
    ...
    isInitialized = true;
    console.log('[WA-Content] Content script core initialized (SPA nav may have issues).');
...
async function reconnectToBackground() {
  if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
    console.error('[WA-Content] Maximum reconnect attempts reached');
    return;
  }
  
  try {
    reconnectAttempts++;
    console.log(`[WA-Content] Attempting to reconnect (${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})`);
    
    // Send a test message to check connection
    const response = await chrome.runtime.sendMessage({ action: 'ping' });
    if (response && response.pong) {
      console.log('[WA-Content] Successfully reconnected to background script');
      reconnectAttempts = 0;
      return true;
    }
    } catch (error) {
    console.error('[WA-Content] Reconnection failed:', error);
    // Schedule next attempt with exponential backoff
    setTimeout(reconnectToBackground, Math.min(1000 * Math.pow(2, reconnectAttempts), 30000));
  }
  return false;
}
...
async function handleCampaignStart(campaignSettings) {
  try {
    ...
    // De-duplicate contacts (skip empty / malformed numbers)
    ...
    (campaignSettings.contacts || []).forEach((c) => {
      const raw = c?.Phone || c?.phone || c?.PHONE;
      const clean = normalizePhone(raw);
      if (!clean || uniqueMap.has(clean)) return; // skip duplicates / invalid
      uniqueMap.add(clean);
...
async function processCampaign() {
  try {
    ...
    const { contacts, message } = activeCampaign;
    ...
    for (let i = activeCampaign.currentIndex; i < contacts.length; i++) {
      ...
      // Determine clean phone; if missing treat as failed and skip
      ...
      try {
        ...
        if (activeCampaign.useLegacyMethod) {
            ...
            if (legacyOk) {
                ...
                console.log('[WA-Content] Legacy send failed, trying UI method as fallback for', cleanPhone);
                ...
                } else if (typeof fallbackResult === 'boolean') { // Should not happen with current sendMessage
                    currentAttemptOk = fallbackResult;
                    if (!currentAttemptOk) attemptError = 'Fallback UI send returned unexpected boolean false';
                } else {
                    currentAttemptOk = false; // Precaution for unexpected return type
                    attemptError = 'Fallback UI send returned unexpected result type';
                }
            }
        } else { // Turbo Mode (non-legacy)
            console.log(`[WA-Content] Using non-legacy method for ${cleanPhone}`);
            // This 'sendMessage' is the global UI one.
            // The 'else' path (fallback to legacy if sendMessage not function) is removed as sendMessage should always be defined.
            ...
            } else if (typeof sendAttemptResult === 'boolean') { // Should not happen with current sendMessage
        ...
        // Turbo Mode – keep the pipeline almost gap-less unless the user enabled random delays/batches
        if (activeCampaign.turboModeEnabled && !activeCampaign.randomTimeGapEnabled && !activeCampaign.delayBetweenBatches) {
          desiredGap = 200; // just 200 ms head-room so WA queues the next message cleanly
        }
        ...
        console.error('[WA-Content] Error processing contact:', error);
...
async function waitForMessageSent(timeout = 5000) {
  return new Promise((resolve, reject) => {
    const startTime = Date.now();
    
    const checkMessageStatus = () => {
      const messageStatus = document.querySelector('span[data-testid="msg-dblcheck"]');
      if (messageStatus) {
        resolve();
          return;
      }
      ...
      setTimeout(checkMessageStatus, 100);
    };
...
async function legacySendMessage(contact, textTemplate, attach) {
  try {
    ...
    // Get the phone number for validation
    ...
    while (!navigated && navigationAttempts < maxNavigationAttempts) {
      ...
      try {
        if (attach) {
          // For attachments, we first navigate to the chat then send the attachment
          console.log(`[WA-Content] Navigating to chat ${phoneNumber}, attempt ${navigationAttempts}`);
          
          // Navigate to chat first (with empty text to avoid auto-filling)
          await createMessageLink(phoneNumber, '');
          
          // Wait until chat panel opens or invalid dialog appears
          const navResult = await waitForChatOrInvalid(12000, phoneNumber);
          if (navResult.invalid) {
            console.warn('[WA-Content] Invalid/blocked number detected right after navigation – skipping');
            return false;
          }
          
          // Small delay to allow chat panel to stabilise
          await new Promise(r => setTimeout(r, 400));
          
          // Guard against late invalid-dialog that may appear after panel loads
          const lateInvalid1 = await waitForInvalidNumberDialog(2000);
          if (lateInvalid1) {
            console.warn('[WA-Content] Invalid/blocked dialog appeared late – skipping');
            return false;
          }
          
          // Now we can try to send the attachment with the caption
          console.log(`[WA-Content] Sending attachment via clipboard to ${phoneNumber}`);
          const success = await sendAttachmentViaClipboard(attach, plainText);
          sendAttempted = true;
          if (!success) {
            console.warn('[WA-Content] Clipboard attachment approach failed, trying alternative');
            // Try an alternative method before giving up
            try {
              const altSuccess = await sendAttachmentSilently(attach, plainText);
              if (altSuccess) {
                console.log('[WA-Content] Silent attachment alternative succeeded');
                sendSuccess = true;
                navigated = true;
                break;
              }
            } catch (altErr) {
              console.warn('[WA-Content] Silent attachment alternative also failed:', altErr);
            }
            throw new Error('All attachment approaches failed');
          }
     ...
     if (sendSuccess && !attach) {
       try {
         const messageSent = await waitForMessageSent(8000);
         if (messageSent) {
           ...
           sendSuccess = false; // will propagate failure
         }
       } catch (e) {
         console.warn('[WA-Content] Error waiting for message sent confirmation:', e);
         sendSuccess = false; // treat as failed
       }
     }
...
async function sendAttachmentViaClipboard(fileObj, caption = '') {
  try {
    console.log('[WA-Content] FILEINPUT: Starting attachment flow. File:', fileObj, 'Caption:', caption);
    // Video attachments are now allowed – removed previous early return.

    // Decide whether we should attempt the fully silent path. On Windows/macOS sending an image
    // via the silent path sometimes routes through the hidden "New sticker" input or otherwise
    // creates a preview before the clip-menu is opened – leading WhatsApp to treat it as a sticker.
    const isLinuxEnv = navigator.platform?.toLowerCase().includes('linux') || navigator.userAgent.toLowerCase().includes('linux');
    const turboFastMode = (typeof activeCampaign !== 'undefined' && activeCampaign?.turboModeEnabled);
    const treatSilentAllowed = isLinuxEnv || turboFastMode;
    const isImageFileGlobal = fileObj.type.startsWith('image/');
    if (!isImageFileGlobal || treatSilentAllowed) {
      // NEW APPROACH: Try to send silently without opening the attachment popup menu
      try {
      ...
      console.log('[WA-Content] FILEINPUT: Skipping silent-attachment path for image on non-Linux to avoid sticker conversion.');
    }
    ...
    try {
      ...
      if (isVid) {
        // We rely on the normal clip menu for videos to avoid duplicate preview injections
    ...
    // 6) Wait briefly for the preview composer.  When Turbo is on we shorten this drastically.
    const turboFast = (typeof activeCampaign !== 'undefined' && activeCampaign?.turboModeEnabled);
    const previewDelay = turboFast ? (fileObj.type.startsWith('image/') ? 400 : 300) : (fileObj.type.startsWith('image/') ? 2000 : 1500);
    await new Promise(res => setTimeout(res, previewDelay));

    // 7) Insert caption if provided.
    if (caption) {
      ...
      for (const sel of captionSelectors) {
        const el = document.querySelector(sel);
        if (el && el.offsetParent !== null) { captionInput = el; break; }
      }
      if (captionInput) {
        captionInput.focus();
        ...
        console.log('[WA-Content] FILEINPUT: Caption set.');
      }
    }

    // 8) Wait for the send button then click it.
    const sendSelectors = [
      'button[data-testid="send"]',
      'span[data-icon="send"]',
    ...
    // Allow up to 60s for large video files to finish processing / uploading
    const maxWaitMs = fileObj.type.startsWith('video/') ? 60000 : 8000;
    ...
    while (!clicked && Date.now() - waitStart < maxWaitMs) {
      for (const sel of sendSelectors) {
        ...
        if (btn && typeof btn.click === 'function') {
          btn.click();
          clicked = true;
          console.log('[WA-Content] FILEINPUT: Send button clicked using selector', sel);
          break;
        }
      }
      if (!clicked) {
        const turboFastPoll = (typeof activeCampaign !== 'undefined' && activeCampaign?.turboModeEnabled) ? 200 : 400;
        await new Promise(r => setTimeout(r, turboFastPoll));
      }
    }

    // 9) Image thumbnail issue - WhatsApp doesn't always show thumbnails properly. Let's help it.
    if (fileObj.type.startsWith('image/')) {
      try {
        console.log('[WA-Content] FILEINPUT: Setting up post-send thumbnail fix for chat history');
        
        // After sending, set up a delayed check for thumbnails in the chat history
        setTimeout(async () => {
          try {
            // Look for common patterns of broken image thumbnails in chat
            const brokenThumbnailSelectors = [
              // Various selectors that might contain empty thumbnails
              'div[data-testid="image-thumb"] img[src=""]',
              'div[data-testid="media-canvas"] img[src=""]',
              '.media-viewer img[src=""]',
              'div[data-testid="document-thumb"] + div:not(:empty)',
              '.document-thumbnail:empty',
              // The generic document icon that appears when image fails to load
              'div[data-icon="document-generic"]',
              'span[data-icon="document-generic"]'
            ];
            
            // Look for messages with "media" class but no proper image thumb
            const mediaWrappers = document.querySelectorAll('.message-in .media, .message-out .media');
            
            let thumbFixed = false;
            
            // Try to fix each potential broken thumbnail
            for (const selector of brokenThumbnailSelectors) {
              const elements = document.querySelectorAll(selector);
              if (elements.length > 0) {
                console.log(`[WA-Content] FILEINPUT: Found ${elements.length} potentially broken thumbnails with selector: ${selector}`);
                
                for (const el of elements) {
                  // For images with empty src, try to set src to a data URL
                  if (el.tagName === 'IMG' && (!el.src || el.src === '')) {
                    try {
                      // If we have a thumbnail from earlier, use it
                      const thumbDataUrl = await generateThumbnail(fileObj);
                      if (thumbDataUrl) {
                        el.src = thumbDataUrl;
                        thumbFixed = true;
                        console.log('[WA-Content] FILEINPUT: Fixed broken image thumbnail with data URL');
                      }
                    } catch (err) {
                      console.error('[WA-Content] FILEINPUT: Error fixing thumbnail src:', err);
                    }
                  }
                  ...
                  try {
                    ...
                    // Try to force redraw by modifying classes
                    const parentDiv = el.closest('div[role="button"]') || el.closest('div');
                    if (parentDiv) {
            ...
            // Additional fix for media wrappers
            if (mediaWrappers.length > 0) {
              console.log(`[WA-Content] FILEINPUT: Found ${mediaWrappers.length} media wrappers that might need help`);
              
              for (const wrapper of mediaWrappers) {
                // Check if this wrapper might be for our recently sent file
                const img = wrapper.querySelector('img');
                if (!img || !img.src || img.src === '') {
                  // This might be our broken thumbnail - try to force redraw
                  wrapper.style.display = 'none';
                  await new Promise(r => setTimeout(r, 30));
                  wrapper.style.display = '';
                  
                  // Try to hack in a thumbnail if possible (on the next iteration of browser rendering)
                  setTimeout(async () => {
                    const thumbDataUrl = await generateThumbnail(fileObj);
                    if (thumbDataUrl) {
                      // Try to find any image element inside this wrapper
                      const imgElements = wrapper.querySelectorAll('img');
                      for (const imgEl of imgElements) {
                        if (!imgEl.src || imgEl.src === '') {
                          imgEl.src = thumbDataUrl;
                          console.log('[WA-Content] FILEINPUT: Fixed media wrapper thumbnail with data URL');
                        }
                      }
                      
                      // If no image element found, try to add one
                      if (imgElements.length === 0) {
                        const newImg = document.createElement('img');
                        newImg.src = thumbDataUrl;
                        newImg.style.maxWidth = '100%';
                        newImg.style.maxHeight = '100%';
                        newImg.style.objectFit = 'contain';
                        
                        // Find a place to insert it
                        const insertTarget = wrapper.querySelector('div:empty') || wrapper;
                        insertTarget.appendChild(newImg);
                        console.log('[WA-Content] FILEINPUT: Injected new thumbnail into media wrapper');
                      }
                    }
                  }, 50);
                }
            ...
            if (!thumbFixed) {
              console.log('[WA-Content] FILEINPUT: No broken thumbnails found to fix or fixes were unsuccessful');
            }
          } catch (err) {
            console.error('[WA-Content] FILEINPUT: Error in post-send thumbnail fix:', err);
          }
        }, 2000); // Wait 2 seconds after sending to check thumbnails
      } catch (err) {
        console.error('[WA-Content] FILEINPUT: Error setting up post-send thumbnail fix:', err);
      }
    }

    return true;
  } catch (err) {
    console.error('[WA-Content] FILEINPUT: Error during attachment flow:', err);
    return false;
  }
}
...
async function sendAttachmentSilently(fileObj, caption = '') {
  try {
    ...
    // Allow silent attachment attempt for video files as well.
    if(fileObj.type.startsWith('video/')){
      console.warn('[WA-Content] SILENT-ATTACH: Attempting silent attach of video file.');
      // Previously returned false here; let the flow continue so fallback mechanisms can run.
    }
...
async function silentImageAttachmentViaPaste(fileObj, caption = '') {
  try {
    ...
    try {
      ...
      const turboFast = (typeof activeCampaign !== 'undefined' && activeCampaign?.turboModeEnabled);
      await new Promise(r => setTimeout(r, turboFast ? 350 : 1000));
      ...
      if (caption) {
        ...
        const captionInput = document.querySelector(
          [
            'div[contenteditable="true"][data-testid="media-caption-input"]',
            'div[contenteditable="true"][aria-label*="caption"]',               // aria-label="Add a caption"
            'div[contenteditable="true"][aria-placeholder*="caption"]',        // aria-placeholder variant
            'div[role="dialog"] div[contenteditable="true"][data-lexical-editor="true"]',
            'div[role="dialog"] div[contenteditable="true"]',
            // As of mid-2025 WhatsApp removed role="dialog" – fallback to any visible Lexical editor with caption placeholder
            'div[contenteditable="true"][data-lexical-editor="true"][aria-placeholder*="caption"]'
          ].join(', ')
        );
