...
export function initSender(currentSource, newSource, callback) {
  ...
  if (startCampaignBtn) {
    addEventListenerWithCleanup(startCampaignBtn, "click", () => {
      // Get latest parsed data
      chrome.storage.local.get(['parsedData'], (result) => {
        parsedData = result.parsedData || [];
...
async function startDirectCampaign(options) {
  try {
    ...
    let campaign = {
      ...
      randomTimeGapMin: options.settings?.randomTimeGapMin ?? 15,
      randomTimeGapMax: options.settings?.randomTimeGapMax ?? 20,
      ...
      batchSizeMin: options.settings?.batchSize ?? 79,
      batchSizeMax: options.settings?.batchSizeMax || 100,
